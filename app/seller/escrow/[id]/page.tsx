...
export default function SellerEscrowPage() {
    ...
    try {
      channel = supabase
        ...
        .on(
          ...
          (payload: any) => {
            console.debug(
              "[SellerEscrowPage] escrow realtime payload",
              payload,
            );
            if (
      ...
      console.error(
        "[SellerEscrowPage] Failed to set up real-time subscription:",
        e,
      );
  ...
  const handleDeliveryProofChange = async (
  ...
  const handleMarkDelivered = async (file?: File) => {
    ...
    try {
      ...
      const response = await fetch("/api/escrow/mark-delivered", {
        ...
        body: JSON.stringify({
          escrowId: escrow.id,
          deliveryProof: deliveryProofPath,
        }),
      });
      if (response.ok) {
        setSuccess(
          "Successfully marked as delivered! The transaction is now in progress.",
        );
  ...
  const price =
    typeof escrow.price === "number" && !isNaN(escrow.price) ? escrow.price : 0;
  const adminFee =
    typeof escrow.admin_fee === "number" && !isNaN(escrow.admin_fee)
      ? escrow.admin_fee
      : 0;
      ...
      <div className="container mx-auto max-w-2xl">
              ...
              {(() => {
                ...
                if ((escrow as any).status_logs) {
                  if (escrow.status === "waiting_payment") {
                    ...
                    const log = (escrow as any).status_logs.find(
                  ...
                  return (
                    <div className="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                      ...
                      <div className="mt-2 text-sm text-yellow-700">
                        {timerLabel
                          .replace("Time left for ", "")
                          .replace("Time left for you to ", "")
                          .replace(":", "")}
                      </div>
                    </div>
                  );
          ...
          <div className="card mb-6">
            ...
            <div className="flex flex-col gap-2 mb-4">
                      ...
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          if (resp.ok && data.success) {
                            ...
                            setEditError(
                              data.error || "Failed to update escrow.",
                            );
                          }
                        } catch (e) {
              ...
              <button
                className="btn-secondary"
                disabled={
                  actionLoading || !escrow.buyer || !escrow.buyer.telegram_id
                }
                onClick={async () => {
        ...
        {canMarkDelivered && (
          <div className="card mb-6">
            ...
            <p className="text-gray-600 mb-4">
              Upload delivery proof (optional) and mark the transaction as
              delivered. Upload starts automatically when you select a file.
            </p>
            <div className="flex flex-col gap-2 mb-4">
              <input
                type="file"
              ...
              <p className="text-sm text-gray-500">
                Supports: JPEG, PNG, WebP, PDF (max 10MB) - Optional delivery
                proof
              </p>
              {actionLoading && (
                <div className="text-blue-600 text-sm">
                  Uploading and marking as delivered...
                </div>
              )}
              {deliveryProofUrl && (
                <div className="text-green-700 text-sm">
                  ✅ Delivery proof uploaded and marked as delivered!
                </div>
              )}
            </div>
          </div>
                ...
                <div key={log.id} className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="font-medium">
                      {getStatusLabel(log.status as any)}
                    </p>
                    <p className="text-sm text-gray-500">
        ...
        <div className="mt-8">
          ...
          {!escrow.buyer && (
            <div className="text-gray-500 text-sm mt-2">
              Waiting for buyer to join. Messages will be delivered when buyer
              joins.
            </div>
          )}
        </div>
