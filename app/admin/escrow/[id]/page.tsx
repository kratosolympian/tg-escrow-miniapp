...
export default function AdminEscrowDetailPage() {
  ...
  const handleAdminAction = async (action: string, notes?: string) => {
    ...
    if (action === "refund") {
      if (
        !confirm(
          `Are you sure you want to refund this transaction to the buyer?`,
        )
      )
        return;
    }
    ...
    try {
      ...
      if (response.ok && (result.success || result.ok)) {
        setSuccess(
          result.message ||
            `${action.replace("-", " ")} completed successfully`,
        );
  ...
  const getAvailableActions = () => {
    ...
    if (escrow.status === "payment_confirmed" || escrow.status === "delivered")
      actions.push("release-funds", "refund");
    if (
      ...
      <div className="min-h-screen bg-gray-50 p-4 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            The requested escrow transaction could not be found.
          </p>
          <Link href="/admin/dashboard" className="btn-primary">
            ← Back to Dashboard
          </Link>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div>
                      ...
                      {isNaN(Date.parse(escrow.created_at))
                        ? "N/A"
                        : formatDateTime(escrow.created_at)}
                    </div>
                    <div>
                      <span className="text-gray-500">Updated:</span>{" "}
                      {isNaN(Date.parse(escrow.updated_at))
                        ? "N/A"
                        : formatDateTime(escrow.updated_at)}
                    </div>
                    ...
                    {(() => {
                      ...
                      if (escrow.status === "waiting_payment") {
                        ...
                        const log = escrow.status_logs?.find(
                          (l: any) => l.status === "payment_confirmed",
                        );
                        if (log) {
                        ...
                        const log = escrow.status_logs?.find(
                          (l: any) => l.status === "delivered",
                        );
                        if (log) {
                          timerLabel =
                            "Time left for buyer to confirm receipt:";
                          timerEnd = new Date(
                            new Date(log.created_at).getTime() + 5 * 60 * 1000,
                          );
                        }
                      }
                      if (timerLabel && timerEnd) {
                        ...
                        return (
                          <div>
                            <span className="text-gray-500">{timerLabel}</span>{" "}
                            <span className="font-mono font-semibold">{`${h.toString().padStart(2, "0")}:${m.toString().padStart(2, "0")}:${s.toString().padStart(2, "0")}`}</span>
                          </div>
                        );
                      }
                      return null;
                ...
                <div className="border rounded-lg p-4 bg-blue-50 border-blue-200">
                  ...
                  <div className="space-y-1 text-sm">
                    <div>
                      ...
                      {escrow.seller.full_name || "Not set"}
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      {escrow.seller.telegram_id || "Not set"}
                    </div>
                    ...
                    {escrow.seller.bank_name && (
                      <div className="pt-2 border-t border-blue-300 mt-2">
                        ...
                        <div>
                          ...
                          {escrow.seller.account_number}
                        </div>
                        <div>
                          <span className="text-gray-600">Name:</span>{" "}
                          {escrow.seller.account_holder_name}
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className="border rounded-lg p-4 bg-green-50 border-green-200">
                  ...
                  {escrow.buyer ? (
                    <div className="space-y-1 text-sm">
                      <div>
                        <span className="text-gray-500">Name:</span>{" "}
                        {escrow.buyer.full_name || "Not set"}
                      </div>
                      <div>
                        <span className="text-gray-500">Email:</span>{" "}
                        {escrow.buyer.email}
                      ...
                      <div>
                        <span className="text-gray-500">Telegram:</span> @
                        {escrow.buyer.telegram_id || "Not set"}
                      </div>
                      ...
                      {escrow.buyer.bank_name && (
                        <div className="pt-2 border-t border-green-300 mt-2">
                          ...
                          <div>
                            <span className="text-gray-600">Account:</span>{" "}
                            {escrow.buyer.account_number}
                          </div>
                          <div>
                            <span className="text-gray-600">Name:</span>{" "}
                            {escrow.buyer.account_holder_name}
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <p className="text-gray-500 text-sm">
                      No buyer has joined this escrow yet
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="card">
              ...
              <div className="space-y-3">
                ...
                <div className="text-sm text-gray-600">
                  ...
                  {escrow.status === "joined" &&
                    "Buyer joined, waiting for payment"}
                  {escrow.status === "waiting_admin" &&
                    "Payment uploaded, needs admin confirmation"}
                  {escrow.status === "payment_confirmed" &&
                    "Payment confirmed, awaiting delivery"}
                  {escrow.status === "delivered" &&
                    "Delivery confirmed, ready for fund release"}
                  {escrow.status === "completed" &&
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">
                    {(() => {
                      ...
                      return Math.max(
                        1,
                        Math.ceil((Date.now() - created) / (1000 * 3600 * 24)),
                      );
                    })()}
                  </span>
                </div>
                  ...
                  <span className="font-semibold">
                    {(() => {
                      ...
                      const hours = Math.floor(
                        (Date.now() - updated) / (1000 * 3600),
                      );
                      if (hours < 1) {
                        const mins = Math.floor(
                          (Date.now() - updated) / (1000 * 60),
                        );
