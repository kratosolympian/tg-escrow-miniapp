...
export async function POST() {
  try {
    ...
    const { error: addError } = await (supabase as any).rpc("exec_sql", {
        ...
        return NextResponse.json(
          {
            ...
            message:
              "Please run this SQL in Supabase dashboard: ALTER TABLE profiles DROP CONSTRAINT profiles_role_check; ALTER TABLE profiles ADD CONSTRAINT profiles_role_check CHECK (role IN ('buyer', 'seller', 'admin', 'super_admin'));",
            details: updateError,
          },
          { status: 500 },
        );
