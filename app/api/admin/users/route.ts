...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: authList, error: authErr } = await service.auth.admin.listUsers()
    if (authErr) {
    ...
    const { data: profiles } = await (service as any).from('profiles').select('id, email, full_name, role, created_at')
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (profileErr) {
      ...
      try { await service.auth.admin.deleteUser(userId) } catch(e) { /* ignore */ }
