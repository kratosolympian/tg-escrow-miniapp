...
export async function POST(request: NextRequest) {
  ...
  if (isSuperAdmin(email) && action === 'remove') {
      return NextResponse.json({ 
        error: 'Cannot remove super admin privileges from the primary admin account' 
      }, { status: 403 })
    }
...
export async function GET() {
  try {
    ...
    const superAdmin = adminsWithFlags.filter((profile: any) => profile.is_super_admin)
    const regularAdmins = adminsWithFlags.filter((profile: any) => !profile.is_super_admin)
