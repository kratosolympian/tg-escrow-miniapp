...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (process.env.DEBUG)
      console.log(
        `${action === "add" ? "Adding admin to" : "Removing admin from"} user id:`,
        targetUser.id,
      );
    ...
    if (existingProfile) {
      ...
      if (process.env.DEBUG)
        console.log(
          "Profile does not exist for user id:",
          targetUser.id,
...
export async function GET() {
  try {
    ...
    const superAdmins = adminProfiles.filter(
      (profile: any) => profile.role === "super_admin",
    );
    const regularAdmins = adminProfiles.filter(
