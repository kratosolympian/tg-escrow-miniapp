  ...
  try {
    ...
    if (authHeader.toLowerCase().startsWith("bearer ")) {
      try {
        ...
        console.info(
          "Confirm received: auth header session lookup",
          user ? user.id : null,
        );
        if (user) authenticatedUser = { id: user.id };
      } catch (e) {
        console.warn(
          "Confirm received: auth.getUser with Authorization header failed, will try cookies/one-time token",
          e,
        );
      }
    }
    ...
    if (!authenticatedUser) {
      try {
        ...
        console.info(
          "Confirm received: cookie session lookup",
          user ? user.id : null,
        );
        if (user) authenticatedUser = { id: user.id };
      } catch (e) {
        console.warn(
          "Confirm received: cookie-based auth failed, will try one-time token",
          e,
        );
      }
    }
      ...
      if (oneTimeToken) {
        try {
          ...
          if (userId) {
            ...
            console.warn(
              "Confirm received: one-time token present but not valid/expired",
            );
    ...
    // Get escrow
    console.info("Confirm received: acting as profile", {
      id: profile?.id,
      escrowId: escrowIdValidated,
    });
    ...
    if (
      !canTransition(escrow.status as EscrowStatus, ESCROW_STATUS.COMPLETED)
    ) {
      return NextResponse.json(
        {
    ...
    try {
      ...
      console.error(
        "Error sending escrow completion notification:",
        notificationError,
      );
