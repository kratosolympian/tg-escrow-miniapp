...
export async function POST(request: NextRequest) {
  try {
    ...
    const { escrowId, deliveryProof, __one_time_token } =
      markDeliveredSchema.parse(body);
    ...
    if (!authenticatedUser) {
      try {
        const {
          data: { user },
        } = await headerClient.auth.getUser();
        if (user) authenticatedUser = { id: user.id };
      } catch (e) {
        // ignore
      }
    ...
    if (!authenticatedUser) {
      let oneTimeToken =
        __one_time_token || request.headers.get("x-one-time-token") || null;
    ...
    if (
      !canTransition(escrow.status as EscrowStatus, ESCROW_STATUS.IN_PROGRESS)
    ) {
      return NextResponse.json(
        {
