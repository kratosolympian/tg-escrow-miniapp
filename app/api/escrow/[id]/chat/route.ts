...
export async function POST(
  ...
  try {
    ...
    if (
      !canAccessEscrow(profile, {
        seller_id: escrowData2.seller_id || "",
        buyer_id: escrowData2.buyer_id,
      })
    ) {
      return NextResponse.json({ error: "Access denied" }, { status: 403 });
    }
    ...
    if (insertError) {
      if ((process.env.NODE_ENV as string) !== "production") {
        console.error("[DEBUG] Insert chat message failed", {
          insertError,
          insertObj,
        });
      }
