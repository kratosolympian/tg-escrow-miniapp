    ...
    if (currentStatus !== ESCROW_STATUS.WAITING_PAYMENT && !canTransition(currentStatus as any, ESCROW_STATUS.WAITING_PAYMENT)) {
      ...
      try {
        ...
        if (buyerCountErr) {
          ...
          if (count >= 5) { // Temporarily increased for testing
            return NextResponse.json({ error: 'You have reached the maximum number of active transactions (5). Please finish an existing transaction before joining another.' }, { status: 400 })
          }
        }
      } catch (e) {
