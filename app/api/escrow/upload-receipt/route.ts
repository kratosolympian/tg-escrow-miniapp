...
export async function POST(request: NextRequest) {
  const makeJson = (
    ...
    try {
      if (process.env.DEBUG) {
        ...
        try {
          payload._debug.serverUser = authenticatedUser
            ? authenticatedUser.id || null
            : null;
        } catch (e) {}
        try {
          payload._debug.escrowId = debugEscrowId || null;
        } catch (e) {}
  ...
  try {
      ...
      if (token) {
        try {
          ...
          if (userId) {
            ...
            console.warn(
              "Upload receipt: one-time token present but not valid/expired",
            );
            return makeJson(
    ...
    if (!receiptFile && process.env.NODE_ENV === "development") {
      ...
      receiptFile = new File([blob], "test-receipt.txt", {
        type: "text/plain",
      });
    }
    ...
    if (escrow.buyer_id !== authenticatedUser.id) {
      return makeJson(
        { error: "Only the buyer can upload receipts" },
        authenticatedUser,
        debugEscrowId,
        403,
      );
    ...
    if (
      escrow.status === ESCROW_STATUS.WAITING_PAYMENT &&
    ...
    if (uploadedFilePath) {
      try {
        ...
        if (process.env.DEBUG)
          console.error(
            "Cleanup error after failed receipt upload:",
            cleanupErr,
          );
      }
    }
