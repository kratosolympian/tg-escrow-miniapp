  ...
  try {
    ...
    if (allError) {
      return NextResponse.json(
        { error: "Failed to fetch admin profiles", details: allError },
        { status: 500 },
      );
    }

    // Filter to those without telegram_id (null or empty string)
    const adminProfiles = allAdmins.filter(
      (profile) => !profile.telegram_id || profile.telegram_id === "",
    );
    ...
    for (
      let i = 0;
      i < Math.min(adminProfiles.length, adminTestIds.length);
      i++
    ) {
