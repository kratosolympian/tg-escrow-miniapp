...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: signed, error: signErr } = await service.storage.from('product-images').createSignedUrl(path, ttlSeconds)
    if (signErr) {
    ...
    return NextResponse.json({ path, signedUrl: signed?.signedUrl || null, expires_at: expiresAt })
  } catch (err) {
