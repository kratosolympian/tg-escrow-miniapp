...
import {
  createSignedToken,
  createSignedTokenAndPersist,
} from "@/lib/signedAuth";

export async function GET(request: NextRequest) {
  try {
    const { createServerClientWithCookies } = await import(
      "@/lib/supabaseServer"
    );
    ...
    if (generateToken) {
      ...
      if (role === "buyer") {
        ...
        const activeBuyerIds = new Set(
          escrows?.map((e) => e.buyer_id).filter(Boolean) || [],
        );
        ...
        const token = await createSignedTokenAndPersist(
          availableBuyer.id,
          3600,
        ); // 1 hour expiry
        return NextResponse.json({
        ...
        const availableSeller = profiles?.find(
          (p) => !activeSellerIds.has(p.id),
        );
