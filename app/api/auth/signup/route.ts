...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn(
        "Signup via server-client failed, will fallback to service role if allowed:",
        err,
      );
      try {
        ...
        if (adminErr) {
          ...
          return NextResponse.json(
            { error: adminErr.message || "Failed to create user" },
            { status: 500 },
          );
        }
        ...
        if (userId) {
          try {
            ...
            console.warn(
              "Failed to create profile via service role after admin.createUser:",
              e,
            );
          }
        }
        ...
        return NextResponse.json(
          { error: (e2 && (e2 as any).message) || "Signup failed" },
          { status: 500 },
        );
      }
    }
    ...
    try {
      ...
      if (signInError) {
        ...
        if (process.env.DEBUG)
          console.log("User signed in after signup, id:", signInData?.user?.id);
      }
    } catch (e) {
    ...
    try {
      ...
      if (process.env.NEXT_PUBLIC_DEBUG)
        console.log(
          "Signup route: created one-time token for user",
          authData.user.id,
        );
      ...
      if (process.env.NEXT_PUBLIC_DEBUG)
        console.log(
          "Signup route: created user but failed to create one-time token, user id=",
          authData.user.id,
        );
      const resp = NextResponse.json({
        user: {
