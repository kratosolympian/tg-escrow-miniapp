    ...
    if (contentType.includes("application/json")) {
      ...
      try {
        ...
        if (
          user &&
          authData.session &&
          (authData.session as any).access_token
        ) {
          // Create a signed one-time token as well so JSON clients always receive a token
          try {
            const { createSignedTokenAndPersist } = await import(
              "@/lib/signedAuth"
            );
            ...
            setAuthCookies(
              resp,
              (authData.session as any).access_token,
              (authData.session as any).refresh_token,
            );
            return resp;
          } catch (e) {
        ...
        console.warn(
          "Could not attach session cookie to JSON login response:",
          e,
        );
      }
