...
export default function BuyerEscrowPage() {
  ...
  const [deliveryProofSignedUrl, setDeliveryProofSignedUrl] = useState<
    string | null
  >(null);
  ...
  const fetchEscrow = async () => {
    ...
    try {
      const resp = await fetch(
        `/api/escrow/by-id/${encodeURIComponent(String(codeStr))}`,
        { credentials: "include" },
      );
      if (!resp.ok) {
        ...
        setEscrow(null);
        if (resp.status === 403)
          console.warn("[dev] escrow fetch returned 403");
      } else {
  ...
  useEffect(() => {
    ...
    const fetchReceiptImages = async () => {
      if (!Array.isArray(escrow.receipts) || escrow.receipts.length === 0)
        return;
  ...
  useEffect(() => {
    ...
    const fetchDeliveryProofSignedUrl = async () => {
      try {
        ...
        if (
          (escrow as any)?.delivery_proof_signed_url &&
          (escrow as any).delivery_proof_signed_url.startsWith("http")
        ) {
        ...
        const response = await fetch("/api/storage/sign-url", {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            path: escrow.delivery_proof_url,
            bucket: "product-images",
          }),
          credentials: "include",
        });
  ...
  useEffect(() => {
    ...
    if (escrow.status === "waiting_payment") {
      ...
      if ((escrow as any).expires_at) {
        ...
        timerEnd = new Date(
          new Date(escrow.created_at || "").getTime() + 30 * 60 * 1000,
        );
      }
      timerLabel = "Time left to complete payment:";
    } else if (
      escrow.status === "in_progress" &&
      confirmationTimeLeft !== null
    ) {
    ...
    if (timerEnd) {
      const updateTimer = () => {
        ...
        const secs = Math.max(
          0,
          Math.floor((timerEnd!.getTime() - now) / 1000),
        );
        setTimeLeft(secs);
  ...
  useEffect(() => {
    if (
      escrow?.status === "in_progress" &&
      escrow.delivery_proof_url &&
      currentUser?.id === escrow.buyer_id
    ) {
              ...
              error.toLowerCase().includes("access denied") && (
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mt-4 rounded">
                  <div className="mb-2 text-sm text-yellow-800">
                    Dev info: Access denied when fetching escrow.
                  </div>
                  <div className="mb-2 text-xs text-gray-700">
                    currentUser.id:{" "}
                    <code>{currentUser?.id || "not-signed-in"}</code>
                  </div>
                  <div className="flex gap-3">
                    <button
                      ...
                      onClick={async () => {
                        try {
                          const resp = await fetch(
                            `/api/dev/escrow/by-code?code=${encodeURIComponent(Array.isArray(code) ? code[0] : code)}`,
                            { credentials: "include" },
                          );
        ...
        {!loading && !error && escrow && (
          <div className="bg-white shadow rounded-lg p-8 mb-8">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
              ...
              <span
                className={`inline-flex items-center gap-2 px-4 py-1 rounded-full text-base font-semibold border-2 ${getStatusColor((escrow.status as any) || "created")}`}
              >
            ...
            <div className="space-y-4">
              <div>
                ...
                {(escrow.description as any) ||
                  escrow.product_description ||
                  "-"}
              </div>
              <div>
                <span className="font-semibold">Product Price:</span>{" "}
                {(escrow.price ?? escrow.amount) !== null &&
                (escrow.price ?? escrow.amount) !== undefined
                  ? formatNaira((escrow.price ?? escrow.amount) as number)
                  : "-"}
              </div>
              {escrow.admin_fee && (
                <div>
              ...
              {escrow.admin_fee && (escrow.price ?? escrow.amount) && (
                <div className="border-t pt-2 mt-2">
                  <span className="font-bold text-lg">Total Amount:</span>{" "}
                  {formatNaira(
                    ((escrow.price ?? escrow.amount) as number) +
                      escrow.admin_fee,
                  )}
                </div>
              )}
              ...
              {bankDetails && (
                <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <div className="mt-3 text-xs text-blue-600">
                    Please make payment to the above account and upload your
                    receipt below.
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                {Array.isArray(escrow.receipts) &&
                escrow.receipts.length > 0 ? (
                  <div className="mt-2">
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    {currentUser &&
                      currentUser.id === escrow.buyer_id &&
                      canUploadReceipt(escrow.status) && (
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm text-gray-500">
                            Uploading will automatically update the transaction
                            status.
                          </span>
                        </div>
                      )}
                    ...
                    {currentUser && currentUser.id !== escrow.buyer_id && (
                      <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded">
                        ...
                        <div className="text-sm text-gray-700 mt-1">
                          ...
                          <code>{escrow.buyer?.email || escrow.buyer_id}</code>.
                          To upload a receipt, sign in as that buyer account.
                        </div>
                          ...
                          <button
                            ...
                            onClick={async () => {
                              try {
                                ...
                                alert(
                                  "Sign out failed. Please clear cookies and try again.",
                                );
                              }
                            }}
                          >
                          ...
                          <button
                            ...
                            onClick={() => {
                              try {
                                ...
                                alert(
                                  "Buyer email copied to clipboard: " + email,
                                );
                              } catch (e) {
                                // eslint-disable-next-line no-console
                                console.error(e);
                            ...
                            <button
                              ...
                              onClick={async () => {
                                try {
                                  const email = escrow.buyer?.email || "";
                                  if (!email)
                                    return alert("No buyer email available");
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    if (!email)
                                      return alert("No seller email available");
                                    const resp = await fetch(
                                      "/api/dev/signin",
                                      {
                                        method: "POST",
                      ...
                      confirmationTimeLeft !== null && (
                        <div className="mt-4 p-4 bg-green-50 border border-green-200 rounded-lg">
                          ...
                          <p className="text-sm text-green-700 mb-3">
                            The seller has marked this transaction as delivered.
                            Please review the delivery proof above and confirm
                            receipt within the time limit.
                          </p>
                          <div className="flex items-center justify-between">
                            <div className="text-sm">
                              ...
                              <span
                                className={`font-mono ${confirmationTimeLeft < 60 ? "text-red-600" : "text-green-600"}`}
                              >
                                {Math.floor(confirmationTimeLeft / 60)}:
                            ...
                            <button
                              ...
                              disabled={
                                confirmingDelivery || confirmationTimeLeft === 0
                              }
                          ...
                          {confirmationTimeLeft === 0 && (
                            <p className="text-sm text-red-600 mt-2">
                              Time limit expired. Please contact support if you
                              have issues with the delivery.
                            </p>
                          )}
                        </div>
                      )}
