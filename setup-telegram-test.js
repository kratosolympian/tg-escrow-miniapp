...
async function setupTelegramForTesting() {
  ...
  for (const profile of profiles) {
    if (!profile.telegram_id && idIndex < testTelegramIds.length) {
      ...
      if (updateError) {
        ...
        console.log(
          `✅ Assigned Telegram ID ${testId} to ${profile.full_name} (${profile.role})`,
        );
      }
  ...
  console.log("\n🎉 Setup complete!");
  console.log(
    "📱 Test Telegram IDs assigned. Now all users should receive notifications.",
  );
  console.log(
    "🧪 Test the notifications at: http://localhost:3000/api/test-notifications",
  );
  ...
  updatedProfiles.forEach((profile) => {
    console.log(
      `- ${profile.full_name} (${profile.role}): ${profile.telegram_id || "❌ NOT SET"}`,
    );
  });
}
