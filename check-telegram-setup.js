...
async function checkProfiles() {
  ...
  data.forEach((profile, i) => {
    console.log(
      `${i + 1}. ${profile.full_name || "No name"} (${profile.role})`,
    );
  ...
  escrows.forEach((escrow, i) => {
    ...
    console.log(
      `   Seller: ${escrow.seller?.full_name || "N/A"} (TG: ${escrow.seller?.telegram_id || "❌ Not set"})`,
    );
    console.log(
      `   Buyer: ${escrow.buyer?.full_name || "N/A"} (TG: ${escrow.buyer?.telegram_id || "❌ Not set"})`,
    );
    console.log("");
  });
  ...
  const escrowsWithTelegram = escrows.filter(
    (e) => e.seller?.telegram_id || e.buyer?.telegram_id,
  );
  ...
  console.log(
    `- Escrows with Telegram recipients: ${escrowsWithTelegram.length}`,
  );
  ...
  if (profilesWithTelegram.length === 0) {
    ...
    console.log(
      "1. Use the Telegram auth flow to associate Telegram IDs with user accounts",
    );
    console.log(
      "2. Or manually set telegram_id in the profiles table for testing",
    );
  } else {
