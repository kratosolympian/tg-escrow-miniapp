...
async function checkUsersAndEscrows() {
  ...
  profiles.forEach((profile, i) => {
    console.log(
      `${i + 1}. ${profile.full_name || "No name"} (${profile.role}) - ID: ${profile.id}`,
    );
  ...
  escrows.forEach((escrow, i) => {
    ...
    console.log(
      `   Seller: ${seller?.full_name || "Unknown"} (TG: ${seller?.telegram_id || "❌"})`,
    );
    console.log(
      `   Buyer: ${buyer?.full_name || "No buyer"} (TG: ${buyer?.telegram_id || "❌"})`,
    );
  });
  ...
  console.log(
    `- Sellers: ${sellers.length} (${sellers.filter((s) => s.telegram_id).length} with Telegram)`,
  );
  console.log(
    `- Buyers: ${buyers.length} (${buyers.filter((b) => b.telegram_id).length} with Telegram)`,
  );
  console.log(
    `- Admins: ${admins.length} (${admins.filter((a) => a.telegram_id).length} with Telegram)`,
  );
  ...
  if (escrows.length > 0) {
    ...
    console.log(
      "2. Get their Telegram user IDs (from Telegram WebApp or manually)",
    );
