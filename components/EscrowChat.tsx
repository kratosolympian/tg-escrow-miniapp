...
export default React.memo(function EscrowChat({
  escrowId,
  currentUserId,
  isAdmin = false,
  supabaseClient,
  ...
  const loadMessages = React.useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        const missing = (data.messages as ChatMessage[])
          .filter((m) => !m.sender || !m.sender.full_name)
          .map((m) => m.id);
        if (missing.length) {
  ...
  const chatMessages: any[] = messages
    ...
    .map((message) => {
      ...
      const sender =
        message.sender ||
        (message.sender_id ? senderCache[message.sender_id] : null);
      ...
      return {
        ...
        type: "text" as const,
        position: (message.sender_id === currentUserId ? "right" : "left") as
          | "right"
          | "left",
            ...
            <div className="flex">
              ...
              <div className="text-sm">
                ...
                <p className="text-amber-700">
                  Keep all communication within this chat. Taking conversations
                  outside the escrow system means admin cannot provide
                  assistance or intervention if disputes arise.
                </p>
              </div>
            </div>
          </div>
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-500">
              ...
              <p className="text-sm text-center mt-1">
                Start the conversation to discuss transaction details
              </p>
            </div>
          ) : (
            <div className="p-3 space-y-3">
              ...
              {systemMessages.map((message) => (
                <div
                  key={message.id}
                  className="bg-gray-100 text-gray-600 text-center py-2 px-3 rounded-lg text-sm"
                >
                  {message.message}
                </div>
              ))}
        ...
        {isAdmin && (
          <div className="border-t border-gray-200 px-4 py-3 bg-gray-50">
            <p className="text-sm text-gray-600 text-center">
              👁️ Admin monitoring mode - You can view all messages but cannot
