...
Product Price: ₦${escrow.price.toLocaleString()}
Service Fee: ₦${(escrow.admin_fee || 0).toLocaleString()}
Total Amount: ₦${(escrow.price + (escrow.admin_fee || 0)).toLocaleString()}
    ...
    const emailHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
          ...
          <p><strong>Product Price:</strong> ₦${escrow.price.toLocaleString()}</p>
          <p><strong>Service Fee:</strong> ₦${(escrow.admin_fee || 0).toLocaleString()}</p>
          <p><strong>Total Amount:</strong> ₦${(escrow.price + (escrow.admin_fee || 0)).toLocaleString()}</p>
...
export function deriveEmailAndPassword(telegramId: string): {
  email: string;
  password: string;
} {
      ...
      case "sendgrid":
        response = await fetch("https://api.sendgrid.com/v3/mail/send", {
          ...
          body: JSON.stringify({
            ...
            from: {
              email: process.env.EMAIL_FROM || "noreply@escrow-service.com",
            },
            subject,
            content: [
              {
