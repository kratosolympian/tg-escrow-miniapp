...
async function persistTokenToDb(
  ...
  try {
    ...
    if (res && (res as any).error) {
      console.warn(
        "persistTokenToDb: insert returned error",
        (res as any).error,
      );
      return false;
    }
    ...
    const insertedCount = Array.isArray(inserted)
      ...
      : inserted
        ...
        : 0;
    if (insertedCount === 0) {
      console.warn("persistTokenToDb: insert returned no rows", {
        tokenId,
        userId,
        res,
      });
...
async function consumeTokenFromDb(tokenId: string) {
  try {
    ...
    try {
      const rpcRes = await (svc as any).rpc("consume_one_time_token", {
        p_id: tokenId,
      });
    ...
    try {
      ...
      if (sel && (sel as any).data && (sel as any).data.user_id) {
        ...
        try {
          const delRes = await (svc as any)
            .from("one_time_tokens")
            .delete()
            .eq("id", tokenId);
          if (delRes && (delRes as any).error) {
            console.warn(
              "consumeTokenFromDb: delete returned error",
              (delRes as any).error,
            );
          }
        } catch (delErr) {
          console.warn(
            "consumeTokenFromDb: delete after select failed",
            delErr,
          );
        }
        return userId;
...
export async function createSignedTokenAndPersist(
  userId: string,
  ttlSeconds = 300,
) {
  const tokenId = crypto.randomUUID();
  ...
  try {
    ...
    if (ok) {
      ...
      console.warn(
        "createSignedTokenAndPersist: token persistence to DB reported failure, will rely on in-memory store",
        { tokenId, userId },
      );
    }
  } catch (e) {
    console.warn(
      "createSignedTokenAndPersist: failed to persist token to DB, proceeding with in-memory only",
      e,
    );
  }
  return token;
