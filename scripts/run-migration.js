...
const MIGRATION_SQL_PATH = new URL('../SQL/2025-09-26-replace-is_admin-with-admin_users.sql', import.meta.url).pathname;

async function main() {
  const dbUrl = process.env.SUPABASE_DB_URL || process.env.DATABASE_URL;
  if (!dbUrl) {
    console.error('ERROR: SUPABASE_DB_URL (or DATABASE_URL) not set. Provide a Postgres connection string with a service-role user.');
    process.exit(2);
  }
  ...
  try {
    ...
    console.log('Running safe seed: insert admin users from profiles where role = \"admin\"');
    await client.query(
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1].endsWith('run-migration.js')) {
  main();
}
