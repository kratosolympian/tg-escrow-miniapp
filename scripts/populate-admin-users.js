...
async function fetchAdminProfiles() {
  ...
  const headers = { apikey: SERVICE_ROLE, Authorization: `Bearer ${SERVICE_ROLE}`, Accept: 'application/json' }
...
async function insertAdminUser(id) {
  ...
  const headers = { apikey: SERVICE_ROLE, Authorization: `Bearer ${SERVICE_ROLE}`, 'Content-Type': 'application/json', Accept: 'application/json' }
...
async function main() {
  try {
    ...
    console.log(`Found ${admins.length} admin profiles. Inserting into admin_users...`)
    let inserted = 0
    ...
    console.log(`Done. Inserted ${inserted} rows. Total admins processed: ${admins.length}`)
    process.exit(0)
  } catch (err) {
