...
async function fetchEscrowPage(code) {
  const res = await cookieFetch(`${BASE}/api/escrow/by-id/${code}`, {
    method: "GET",
  });
...
async function main() {
  ...
  const signedInId =
    meJson && meJson.user && meJson.user.id ? meJson.user.id : null;
  const escrowBuyerId =
    escrow.body && escrow.body.escrow && escrow.body.escrow.buyer_id
      ? escrow.body.escrow.buyer_id
      : null;
