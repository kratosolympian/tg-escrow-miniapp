...
const SELLER_EMAIL =
  process.env.SELLER_EMAIL || `repro.seller+${Date.now()}@example.com`;
const BUYER_EMAIL =
  process.env.BUYER_EMAIL || `repro.buyer+${Date.now()}@example.com`;
...
async function main() {
  try {
    const seller = await adminCreateUser(
      SELLER_EMAIL,
      TEST_PASSWORD,
      "Repro Seller",
    );
    const buyer = await adminCreateUser(
      BUYER_EMAIL,
      TEST_PASSWORD,
      "Repro Buyer",
    );
    ...
    if (resp.status >= 500) {
      console.error(
        "Server error detected when buyer fetched escrow — possible policy recursion",
      );
      process.exit(2);
    }
