...
for (const resp of responses) {
  ...
  if (resp.status >= 400) {
    ...
    for (const m of possibleMethods) {
      ...
      if (reqMap.has(key)) {
        foundReqHeaders = reqMap.get(key);
        break;
      }
    }
    ...
    fails.push({
      ...
      requestHeaders: foundReqHeaders || null,
      body: resp.body || null,
      text: resp.text || null,
    });
  }
...
if (fails.length > 0) {
  for (let i = 0; i < Math.min(10, fails.length); i++) {
    ...
    console.log(
      ...
      f.status,
      f.url,
      "Cookie present in request headers?",
      f.requestHeaders && (f.requestHeaders.cookie || f.requestHeaders.Cookie)
        ? "yes"
        : "no",
    );
  }
