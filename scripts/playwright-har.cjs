...
(async () => {
  ...
  page.on('response', async (res) => {
    try {
      ...
      responses.push({
        ...
        body: body && body.length > 10000 ? body.slice(0, 10000) + '...[truncated]' : body,
        timestamp: Date.now(),
      });
    } catch (e) {
      // ignore
  ...
  try {
    ...
    await page.addInitScript(() => {
      ...
      try {
        ...
        window.fetch = async function(input, init) {
          try {
            ...
            const credentials = (init && init.credentials) || (typeof input === 'object' && input && input.credentials) || 'omit';
            ...
            try {
              if (typeof input !== 'string' && input && input.headers) Object.assign(headers, headersToObject(input.headers));
            } catch (e) {}
            // if cookie is readable via document.cookie, also include it for convenience
            let docCookie = '(unreadable)';
            try { docCookie = document.cookie || '(empty)'; } catch (e) { docCookie = '(err)'; }
            console.log('[INSTRUMENT][fetch] ->', method, url, { credentials, headers, documentCookie: docCookie });
          } catch (e) {
            console.log('[INSTRUMENT][fetch] error prepping log', e && e.message);
          }
      ...
      try {
        ...
        XMLHttpRequest.prototype.open = function(method, url) {
          ...
          this.__instrument_request_headers = this.__instrument_request_headers || {};
        ...
        XMLHttpRequest.prototype.send = function(body) {
          try {
            ...
            let docCookie = '(unreadable)';
            try { docCookie = document.cookie || '(empty)'; } catch (e) { docCookie = '(err)'; }
            console.log('[INSTRUMENT][xhr] ->', this.__instrument_method || 'GET', this.__instrument_url || '', { headers, body: body ? String(body).slice(0, 200) + (String(body).length > 200 ? '...[truncated]' : '') : undefined, documentCookie: docCookie });
          } catch (e) {
    ...
    const signin = await page.evaluate(async (email) => {
      const resp = await fetch('/api/dev/signin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email }) });
    ...
    try {
      await page.evaluate(() => {
        try {
          console.log('[INSTRUMENT][document.cookie] ->', document.cookie || '(empty)');
        } catch (e) {
      ...
      await page.evaluate(async () => {
        try {
          ...
          const resp = await fetch('/api/storage/sign-url', {
            ...
            credentials: 'include',
            body: JSON.stringify({ path: 'MG171MFW7GRE/receipt-test.jpg', bucket: 'receipts' })
          });
          const j = await resp.json().catch(() => null);
          console.log('[INSTRUMENT][explicit][sign-url] status', resp.status, 'json', j);
        } catch (e) {
        ...
        try {
          ...
          const imageData = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChwGA60e6kgAAAABJRU5ErkJggg=='; // 1x1 transparent PNG
          ...
          console.log('[INSTRUMENT][explicit][upload-receipt] status', resp2.status, 'json', j2);
        } catch (e) {
          console.log('[INSTRUMENT][explicit][upload-receipt] error', e && e.message);
        }
      });
    } catch (e) {
    ...
    try {
      await page.evaluate(() => {
        try {
          console.log('[INSTRUMENT][document.cookie-after-nav] ->', document.cookie || '(empty)');
        } catch (e) {
          console.log('[INSTRUMENT][document.cookie-after-nav] -> error', e && e.message);
        }
      });
    } catch (e) {
      // ignore
    ...
    fs.writeFileSync(responsesPath, JSON.stringify(responses, null, 2));
      fs.writeFileSync(path.join(outDir, 'console.json'), JSON.stringify(consoles, null, 2));

      // Correlate console messages to nearest network event (within 3000ms)
      const correlation = consoles.map(c => {
        ...
        return {
          console: c,
          nearestResponse: nearest ? { url: nearest.r.url, status: nearest.r.status, timestamp: nearest.r.timestamp, requestHeaders: nearest.r.request.headers } : null,
          withinMs: nearest ? nearest.dt : null
        };
      }).filter(entry => entry.withinMs !== null && entry.withinMs <= 3000);

      fs.writeFileSync(path.join(outDir, 'correlation.json'), JSON.stringify(correlation, null, 2));

    const failing = responses.filter(r => r.status >= 400).map(r => ({ url: r.url, status: r.status, requestHeaders: r.request.headers }));
