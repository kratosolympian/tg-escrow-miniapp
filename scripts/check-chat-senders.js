...
if (!fetchImpl) {
  try {
    ...
    console.error('Node does not have global fetch and node-fetch is not installed. Please run: pnpm add -D node-fetch or run on Node 18+.')
    process.exit(2)
  }
}
...
async function main() {
  ...
  const headers = {}
  if (typeof cookieHeader !== 'undefined' && cookieHeader !== null) headers['Cookie'] = cookieHeader
  if (typeof bearerToken !== 'undefined' && bearerToken !== null) headers['Authorization'] = `Bearer ${bearerToken}`
  const res = await fetch(url, { headers, credentials: 'include' })
    ...
    for (const m of missing) {
      console.log(`- id=${m.id} sender_id=${m.sender_id} message="${String(m.message).slice(0,80)}"`)
    }
    process.exit(1)
  } catch (err) {
