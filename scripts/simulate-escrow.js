...
if (!SERVICE_ROLE || !SUPABASE_PROJECT_URL) {
  console.error(
    "Please set SUPABASE_SERVICE_ROLE_KEY (and ensure NEXT_PUBLIC_SUPABASE_URL or SUPABASE_PROJECT_URL is present) in .env.local or environment",
  );
  process.exit(2);
}

const SELLER_EMAIL =
  process.env.SELLER_EMAIL || `test.seller+${Date.now()}@example.com`;
const BUYER_EMAIL =
  process.env.BUYER_EMAIL || `test.buyer+${Date.now()}@example.com`;
...
async function postChat(fetcher, token, escrowId, message) {
  const result = await jsonPost(
    fetcher,
    `${BASE_URL}/api/escrow/${escrowId}/chat`,
    { message },
    token,
  );
  if (result.status !== 200) {
...
async function fetchEscrowAsAuthenticated(id, cookieJar) {
  ...
  const cookies = await new Promise((res) =>
    cookieJar.getCookies(BASE_URL, (e, c) => res(c)),
  );
  const sbTokenCookie = cookies.find(
    (c) => c.key === "sb:token" || c.key === "sb_token" || c.key === "sb-token",
  );
...
async function getChat(fetcher, token, escrowId) {
  const res = await fetcher(`${BASE_URL}/api/escrow/${escrowId}/chat`, {
    method: "GET",
...
async function markDelivered(fetcher, token, escrowId) {
  const result = await jsonPost(
    fetcher,
    `${BASE_URL}/api/escrow/mark-delivered`,
    { escrowId },
    token,
  );
  if (result.status !== 200) {
...
async function confirmReceived(fetcher, token, escrowId) {
  const result = await jsonPost(
    fetcher,
    `${BASE_URL}/api/escrow/confirm-received`,
    { escrowId },
    token,
  );
  if (result.status !== 200) {
...
async function main() {
  try {
    ...
    const seller = await createUserViaSupabaseAdmin(
      SELLER_EMAIL,
      TEST_PASSWORD,
      "Script Seller",
    );
    await assert(seller && seller.id, "Failed to create seller");
    const buyer = await createUserViaSupabaseAdmin(
      BUYER_EMAIL,
      TEST_PASSWORD,
      "Script Buyer",
    );
    ...
    const sellerLogin = await loginWithFetcher(
      sellerFetch,
      SELLER_EMAIL,
      TEST_PASSWORD,
    );
    await assert(
      sellerLogin && (sellerLogin.token || sellerLogin.user),
      "Seller login failed",
    );
    const sellerToken = sellerLogin.token;

    const buyerLogin = await loginWithFetcher(
      buyerFetch,
      BUYER_EMAIL,
      TEST_PASSWORD,
    );
    await assert(
      buyerLogin && (buyerLogin.token || buyerLogin.user),
      "Buyer login failed",
    );
    ...
    console.log(
      "Seller login user object:",
      JSON.stringify(sellerLogin.user || {}, null, 2),
    );
    console.log(
      "Buyer login user object:",
      JSON.stringify(buyerLogin.user || {}, null, 2),
    );

    // Seller creates escrow
    const createResp = await createEscrow(
      sellerFetch,
      sellerToken,
      "Script product",
      500,
    ...
    await assert(
      createResp && createResp.code && createResp.escrowId,
      "Create escrow failed or missing code/escrowId",
    );
    ...
    console.log(
      "Admin escrow after join:",
      JSON.stringify(adminEscrowAfterJoin, null, 2),
    );
    ...
    console.log(
      "Admin escrow after upload:",
      JSON.stringify(adminEscrowAfterUpload, null, 2),
    );
    ...
    console.log(
      ...
      JSON.stringify(adminConfirm, null, 2),
    );
    ...
    console.log(
      "Authenticated user PostgREST response:",
      JSON.stringify(authEscrowResp, null, 2),
    );

    // Buyer sends chat
    // Debug: show buyer cookie jar and try GET first to see access/read response
    const buyerCookiesDetailed = await new Promise((res) =>
      buyerJar.getCookies(BASE_URL, (e, c) => res(c)),
    );
    console.log(
      "Buyer cookie jar strings:",
      buyerCookiesDetailed.map((x) => x.cookieString()),
    );
    console.log(
      "Buyer cookie objects:",
      buyerCookiesDetailed.map((c) => ({
        key: c.key,
        domain: c.domain,
        path: c.path,
        expires: c.expires,
    ...
    console.log(
      "GET /chat before POST:",
      JSON.stringify(getBeforePost, null, 2),
    );
    const chatPost = await postChat(
      buyerFetch,
      buyerToken,
      escrowId,
      "Payment sent (script)",
    ...
    await assert(
      ...
      "Seller fetch chat failed or invalid format",
    );
