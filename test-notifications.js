...
async function testNotifications() {
  ...
  try {
    ...
    escrows.forEach((escrow, i) => {
      ...
      console.log(
        `     Seller: ${escrow.seller?.full_name || "N/A"} (TG: ${escrow.seller?.telegram_id || "N/A"})`,
      );
      console.log(
        `     Buyer: ${escrow.buyer?.full_name || "N/A"} (TG: ${escrow.buyer?.telegram_id || "N/A"})`,
      );
    });

    // Test 2: Try to send a test notification if we have an escrow with telegram IDs
    const testEscrow = escrows.find(
    ...
    if (testEscrow) {
      ...
      console.log(
        "To test notifications, you need users with telegram_id in their profiles",
      );
    }
